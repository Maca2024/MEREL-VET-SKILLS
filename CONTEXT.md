...
7. **A content-based file classifier must have an explicit "doesn't match" outcome, never a silent default to the first/most-plausible category.** `readers.classify_copy_paste_file()` returns `None` when fewer than half a `.txt` file's non-empty lines contain a tab character, so a stray prose file (a LEESMIJ/README) in `--input-dir` is skipped rather than mis-classified as `toeslagen`/`margeregels` — found via a real smoke test against Merel's own unscrubbed Animana test-account export on 8 jul 2026 (a plain README won the toeslagen slot over the real file, alphabetically). Any future content-based auto-classifier in either skill must follow the same pattern: a real "not this category" return path, not a fallback default.
...
8. **A parser must account for every source record it was handed -- what it drops AND what it never saw.** `readers._read_numbered_csv_rows()` parses line by line and reports every line it cannot turn into a record, so `ParseResult.source_row_count` (= parsed + malformed) always equals the number of source records offered. A plain `csv.reader` over the whole file does NOT have this property: on a file whose quoting is broken it stays "inside quotes" past a line boundary and swallows whole blocks into one record, and those rows are never counted as malformed -- they simply cease to exist. That is exactly how the first real client run (9 jul 2026) shipped 2225 of 4519 products in a workbook labelled *compleet*, with only 722 of the 2294 missing products reported. `build_rpa_products.py` now refuses to write a workbook labelled "compleet" when more than `MAX_DROP_RATE` (2%) of the source rows are unparseable; `--allow-partial` is the deliberate override. Never widen the gate's default, and never reintroduce a whole-file `csv.reader` in either skill: silent shrinkage is worse than a loud stop.

---

## 6. Known gaps / open items (be aware, don't silently "fix")

- **GitHub: live and public** -- [Maca2024/MEREL-VET-SKILLS](https://github.com/Maca2024/MEREL-VET-SKILLS), `master` in sync as of 11 jul 2026 (commits `2892081` fix + `5091f91` docs). The earlier contradiction in this section (staging tree had no `.git`) is resolved: the working clone lives on the Kathedraal at `/home/merel/uploads/MEREL-VET-SKILLS`. Pushing needs the Maca2024 token (`/root/nina-root/.env`, `GITHUB_TOKEN`) and the GitHub noreply author e-mail, else GH007 rejects the push.
...
- **That client's export cannot be fully recovered: 269 of its 4519 rows (6.2%) still do not parse** even after the Excel-roundtrip repair, so the completeness gate (invariant 8) correctly refuses to build it. This is not a parser gap to keep chipping away at -- the source file is damaged. The fix for that client is a **fresh export straight out of Animana, never opened in Excel**. Merel's own clean test-account export (3317 rows) parses 100%, which is the evidence that the skill is fine and the intermediate Excel step is the culprit.
- **The `leveranciers` dropdown (column M) overflows the template.** The real client account behind that run has 377 suppliers; `RPA_Products_template.xlsx` pre-provisions **48**. Values beyond 48 are written to the sheet but show no dropdown arrow in Excel. Pre-existing, never raised with Merel, and not touched by the Excel fix -- decide with her whether the template needs widening before "fixing" it here.
- **556 suppliers legitimately do not resolve.** With `--contacts` supplied, 556 `leverancier` values in the product export are still flagged as unknown, because they are not marked `leverancier=true` in the contacts export. That is a data question for the clinic, not a bug -- do not "fix" it by loosening the flagging.
- **`pytest` was not installed on the Kathedraal at all** until 11 jul 2026 (now installed system-wide with `--break-system-packages`, matching how `openpyxl`/`rapidfuzz` were installed). Meaning: before that date the test suite had never actually been executed on the Kathedraal itself. Run `python3 -m pytest tests/ -q` from each skill's own directory (29/29 green for Skill 1 as of 11 jul).
